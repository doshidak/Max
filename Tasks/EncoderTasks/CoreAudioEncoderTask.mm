...
-(void) writeMPEG4Tags
{
	...
	if(MP4Optimize([[self outputFilename] fileSystemRepresentation], [tempFilename fileSystemRepresentation])) {
		...
		if([fileManager removeItemAtURL:[NSURL fileURLWithPath:[self outputFilename]] error:nil]) {
			if(NO == [fileManager moveItemAtURL:[NSURL fileURLWithPath:tempFilename] toURL:[NSURL fileURLWithPath:[self outputFilename]] error:nil]) {
 		...
 		if([[NSUserDefaults standardUserDefaults] boolForKey:@"useiTunesWorkarounds"]) {
			...
			NSAssert(NULL != frame, NSLocalizedStringFromTable(@"Unable to allocate memory.", @"Exceptions", @""));
 			...
 			if(NSNotFound == index)
 				...
 				frame->setText(TagLib::String([[NSString stringWithFormat:@"(%u)", (unsigned int)index] UTF8String], TagLib::String::UTF8));
